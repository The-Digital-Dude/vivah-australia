...
| MATCH-003 Recently Viewed Profiles                         | Complete                  | `profile_views` collection, duplicate-safe upsert on profile view, `/api/me/recently-viewed`, member recently viewed page, route QA entry, and tests for storage plus privacy-filtered response.                                                                                                                                                   |
...
| NOTIF-004 Push Notification Placeholder                    | Complete for sprint scope | Push subscription model, placeholder console/webpush provider abstraction, member push subscription/test endpoints, settings UI action, and tests. Full browser service worker registration and native push delivery remain future work.                                                                                                           |
...
| SEC-004 Fraud Prevention Rules                             | Partial                   | Fraud event model, high-velocity profile-view rule, admin fraud queue, and tests. Repeated reports, duplicate contact attempts, unusual message volume, scoring policy tuning, and automated admin workflows remain.                                                                                                                               |
...
| UI-001 Responsive Layout System                            | Partial                   | Shared UI tokens and current public/member/admin/auth layouts are responsive and route QA covers core pages. Mobile drawers and systematic viewport visual regression coverage remain.                                                                                                                                                             |
| UI-002 Core Component Library                              | Partial                   | `@vivah/ui` now exports typed tokens, class merge helper, page header, metric card, button, text input, empty state, and data table primitives. Select/date/modal/drawer/tabs/toast/avatar/profile card/plan card/skeleton/pagination/file uploader remain.                                                                                        |
...
4. Add saved search UX and broader fraud-rule coverage.
5. Add frontend test coverage for the expanded `/admin/cms` editor and future visual CMS layout controls.
