...
- Public/member frontend alignment sprint FE-001 audit, FE-002 shared design system, FE-003 shared public/member layout, FE-004 clickable profile cards, FE-005 premium public profile detail page, and FE-006 static pages alignment are complete in `FRONTEND_PROGRESS.md` and `UI_UX_TASKLIST.md`; FE-005 profile detail links now resolve by ObjectId, slug, or display ID, signed-in members can view member-only profile URLs with client auth, unauthenticated member-only profiles show a sign-in-required state, FE-006 root-level static public pages pull from CMS APIs with clean Ivory layouts, burgundy typography, and gold accenting, and remaining public/member page redesign work continues from FE-007.
