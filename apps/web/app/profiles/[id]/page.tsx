...
export default async function ProfileViewPage({ params }: ProfilePageProps) {
  ...
  const heroSummary = [
    ...
    .filter(Boolean)
    .join(' | ');
  const lastActive = profile.updatedAt ? new Date(profile.updatedAt).toLocaleDateString() : 'Recently active';
  const membershipBadge = profile.verification?.level === 'FULLY_VERIFIED' ? 'Platinum member' : 'Vivah member';
      ...
      <article className="mx-auto grid max-w-6xl gap-6 pb-28 lg:grid-cols-[1fr_320px] lg:pb-0">
        <div className="grid gap-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Panel title="Basic Details">
              ...
              <Detail label="Height" value={profile.personal?.heightCm ? `${profile.personal.heightCm} cm` : undefined} />
            ...
            <Panel title="Lifestyle">
              ...
              <Detail label="Living arrangement" value={formatEnum(profile.lifestyle?.livingArrangement)} />
            </Panel>
          ...
          <ProfileDetailSection title="Partner Expectations">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <Detail label="Preferred communities" value={joinList(profile.partnerPreference?.communities)} />
              <Detail label="Preferred education" value={joinList(profile.partnerPreference?.educationLevels)} />
            </div>
          </ProfileDetailSection>
        ...
        <div className="fixed inset-x-0 bottom-0 z-30 border-t border-[#7A1F2B]/10 bg-white/95 p-3 shadow-[0_-14px_40px_rgba(122,31,43,0.12)] backdrop-blur lg:hidden">
          <div className="mx-auto grid max-w-5xl gap-2">
            <ProfileActions profileId={profileId} compact />
