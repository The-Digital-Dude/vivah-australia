...
export default function AuthShell({
  ...
  return (
    <div className="min-h-screen bg-[#FCFAF7] text-[#1A1A1A] grid lg:grid-cols-[1.1fr_1fr]">
      {/* Left Emotional Brand Panel (Desktop-only) */}
      <section className="hidden lg:flex flex-col justify-between p-16 text-white relative overflow-hidden bg-gradient-to-br from-[#7A1F2B] via-[#651925] to-[#4A0A14] border-r border-[#7A1F2B]/10">
        {/* Background decorative glowing element */}
        <div className="absolute -top-40 -left-40 size-[500px] rounded-full bg-[#D4AF37]/5 blur-[120px] pointer-events-none" />
        <div className="absolute -bottom-40 -right-40 size-[500px] rounded-full bg-[#7A1F2B]/20 blur-[120px] pointer-events-none" />

        {/* Top Header Branding */}
        <div className="relative z-10 flex items-center gap-3">
          <span className="font-serif font-extrabold text-2xl tracking-wide text-white">
            VIVAH <span className="text-[#D4AF37]">AUSTRALIA</span>
          </span>
        </div>

        {/* Center Marketing & Matrimonial Taglines */}
        <div className="relative z-10 max-w-xl my-auto py-12">
          <div className="w-16 h-1.5 bg-[#D4AF37] rounded-full mb-8 shadow-sm" />
          ...
          <ul className="grid gap-4 mt-8">
            {trustItems.map((item) => (
              <li key={item} className="flex items-center gap-3">
                <div className="size-6 rounded-full bg-[#D4AF37]/10 flex items-center justify-center text-[#D4AF37] shrink-0 border border-[#D4AF37]/20">
        ...
        <div className="relative z-10 bg-white/5 backdrop-blur-md border border-white/15 rounded-3xl p-6 shadow-xl max-w-lg mt-auto">
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="size-8 rounded-full bg-[#D4AF37]/20 flex items-center justify-center text-[#D4AF37] text-xs font-bold font-serif border border-[#D4AF37]/30">
              S
            </div>
            <div>
      ...
      <main className="flex flex-col justify-between min-h-screen bg-[#FCFAF7]">
        <PublicHeader />

        <div className="flex-1 flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
          <section className="w-full max-w-md bg-white border border-[#7A1F2B]/10 rounded-3xl p-8 shadow-[0_18px_50px_rgba(122,31,43,0.06)] sm:p-10 transition-shadow hover:shadow-[0_18px_50px_rgba(122,31,43,0.08)]">
            <h1 className="font-serif font-bold text-2xl text-[#1A1A1A]">{title}</h1>
